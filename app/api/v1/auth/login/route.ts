...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return Response.json({ error: { code: "VALIDATION_ERROR", message: "Enter a valid email and password." } }, { status: 400 });

    const user = await prisma.user.findUnique({ where: { email: parsed.data.email.toLowerCase() } });
    const valid = user?.passwordHash ? await compare(parsed.data.password, user.passwordHash) : false;
    if (!user || !valid || user.suspendedAt || user.deletedAt) return Response.json({ error: { code: "INVALID_CREDENTIALS", message: "Email or password is incorrect." } }, { status: 401 });
